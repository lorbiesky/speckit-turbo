...
function facts(state, definition) { return new Set(state.phases.filter((phase) => phase.status === "completed" && phase.gate.passed).flatMap((phase) => definition.get(phase.id).provides || [])); }
function applies(condition, state, config) { if (!condition) return true; const visual = state.visualAnalysis || {}, inputs = visual.inputs || []; if (condition === "visual_input_present") return config.visual?.enabled !== false && inputs.length > 0; if (condition === "visual_input_absent_or_completed") return !inputs.length || ["completed", "skipped"].includes(visual.status); throw new Error(`Unsupported workflow condition '${condition}'`); }
function advance(state, flow, config) {
  const defs = new Map(flow.phases.map((phase) => [phase.id, phase])); const done = facts(state, defs);
  for (const phase of state.phases) {
    if (["completed", "skipped"].includes(phase.status)) continue; const def = defs.get(phase.id);
    if (!applies(def.condition, state, config)) { phase.status = "skipped"; phase.gate.passed = true; phase.gate.reason = `Condition '${def.condition}' is not applicable.`; if (phase.id === "visual-analysis") state.visualAnalysis.status = "skipped"; continue; }
    ...
    phase.status = "active"; if (phase.id === "visual-analysis") state.visualAnalysis.status = "active"; state.status = "active"; state.currentPhase = phase.id; state.nextAction = `Complete '${phase.id}' and record evidence for every declared gate requirement.`; return;
  }
...
function runtime(args) {
  ...
  if (command === "start") {
    ...
    const flow = workflow(root, classification), inputs = filtered.flatMap((item, index) => item === "--visual-input" ? [filtered[index + 1]] : []).filter(Boolean);
    const state = { version: "1.0", workId, classification, workflow: flow.name, status: "draft", currentPhase: flow.phases[0].id, phases: flow.phases.map((def) => ({ id: def.id, status: "pending", owner: def.owner, artifacts: def.outputs || [], gate: { passed: null, evidence: [], requirements: Object.fromEntries(def.gate.require.map((key) => [key, ""])) } })), decisions: [], openQuestions: [], validations: [], deviations: [], visualAnalysis: { status: "not_started", inputs, visualSpecPath: "visual-spec.md", persistedReferencesPath: ".specify/visual-references", confidence: "low", unresolvedQuestions: [], acceptanceCriteria: [] }, nextAction: "Start the first applicable workflow phase.", updatedAt: now() };
    advance(state, flow, profile(root)); writeJson(stateFile, state); return status(root, state, flow);
  }
  const state = json(stateFile), flow = workflow(root, state.classification), defs = new Map(flow.phases.map((phase) => [phase.id, phase]),); const config = profile(root);
  ...
  if (command === "complete") { const id = filtered[1], phase = phaseById(state, id); if (phase.status !== "active") fail(`Phase '${id}' is ${phase.status}, not active`); const def = defs.get(id), evidence = {}; filtered.forEach((value, index) => { if (value === "--evidence") { const [key, ...rest] = (filtered[index + 1] || "").split("="); evidence[key] = rest.join("="); } }); for (const key of Object.keys(evidence)) if (!def.gate.require.includes(key)) fail(`Evidence references undeclared requirement '${key}'`); Object.assign(phase.gate.requirements, evidence); const missing = def.gate.require.filter((key) => !phase.gate.requirements[key]); if (missing.length) fail(`Missing evidence for: ${missing.join(", ")}`); phase.gate.evidence = Object.entries(phase.gate.requirements).map(([key, value]) => `${key}: ${value}`); phase.gate.passed = true; if (id === "visual-analysis") state.visualAnalysis.status = "completed"; const checkpoint = def.human_checkpoint === "always" || (def.human_checkpoint === "when_configured" && (config.workflow?.human_checkpoints || []).includes(id)); if (checkpoint) { phase.status = "blocked"; phase.checkpoint = { status: "awaiting_approval", note: "Human approval required by project profile." }; state.status = "paused"; state.nextAction = `Approve or reject the human checkpoint for '${id}'.`; } else { phase.status = "completed"; advance(state, flow, config); } writeJson(stateFile, state); return status(root, state, flow); }
  if (command === "checkpoint") { const id = filtered[1], phase = phaseById(state, id); if (phase.checkpoint?.status !== "awaiting_approval") fail(`Phase '${id}' is not awaiting a human checkpoint`); const approved = has(filtered, "--approve"), note = argValue(filtered, "--note") || (approved ? "Approved by human." : "Rejected by human."); phase.checkpoint = { status: approved ? "approved" : "rejected", note }; if (approved) { phase.status = "completed"; advance(state, flow, config); } else { phase.status = "blocked"; state.status = "blocked"; state.currentPhase = id; state.nextAction = "Address the rejection, then resume."; } writeJson(stateFile, state); return status(root, state, flow); }
  ...
  fail("workflow commands: start, status, complete, checkpoint, resume");
}
function status(_root, state, flow) { const phase = phaseById(state, state.currentPhase), def = flow.phases.find((item) => item.id === phase.id); console.log(`${state.classification} / ${state.status} / ${state.currentPhase}\nOwner: ${phase.owner}\nGate requirements: ${def.gate.require.join(", ")}\nNext action: ${state.nextAction}`); }
function doctor(args) {
  ...
  if (exists(path.join(turbo, "project.yml"))) { const config = profile(root); if (String(config.project?.name || "").startsWith("change-me")) warn("project.yml still contains template placeholders"); } else error("Missing .specify/turbo/project.yml");
